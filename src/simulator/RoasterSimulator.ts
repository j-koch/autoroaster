...
declare global {
  interface Window {
    ...
    gameAPI?: {
      ...
      isRoasting(): boolean;
      updateActualProfile(temp: number): void;
      getGameMode(): 'lookahead' | 'blind';
    };
...
/**
 * ONNX Session collection for the roaster models
 */
...
/**
 * Forecast data storage structure
 */
...
/**
 * Main Roaster Simulator Class
 * 
 * Manages the complete roasting simulation including:
 * - ONNX model inference for physics simulation
 * - Control input processing (manual, PID, neural)
 * - Data collection and visualization
 * - UI state management
 */
export class RoasterSimulator {
  ...
  private readonly timestep: number = 1.5; // Fixed timestep in seconds (for physics calculations)
  private speedupFactor: number = 8; // Speedup multiplier (1x = real-time, 2x = double speed, etc.) - Default to 8x
  ...
  private previousBeanTemp: number = 25.0;
  private previousTime: number = 0;
  
  // Simulation time tracking (separate from wall-clock time)
  private simulationTime: number = 0; // Track simulation time in seconds
  private stepCount: number = 0;      // Count of simulation steps taken
  
  // Background reference profile (randomized on load)
  // This will be initialized after models load
  private backgroundProfile: BackgroundProfile | null = null;
  
  // Controller mode and instances
  private controlMode: ControlMode = 'manual';
  private pidController: PIDController | null = null;
  private neuralController: NeuralController | null = null;
  private neuralControllerConfig: any = null; // Neural controller metadata
  
  // Slider elements for UI control
  private sliderElements: SliderElements | null = null;
  ...
  private initializeUI(): void {
    // Control sliders - safely get elements with type assertions
    const heaterSlider = document.getElementById('heater-slider') as HTMLInputElement | null;
    const fanSlider = document.getElementById('fan-slider') as HTMLInputElement | null;
    const massSlider = document.getElementById('mass-slider') as HTMLInputElement | null;
    const ambientSlider = document.getElementById('ambient-slider') as HTMLInputElement | null;
    const speedupSelect = document.getElementById('speedup-select') as HTMLSelectElement | null;
    ...
    if (massSlider && massValue) {
      this.sliderElements = {
        mass: massSlider,
        massValue: massValue,
        massStatus: document.getElementById('mass-status') as HTMLElement
      };
    }
    
    // Control mode selector
    const controlModeSelect = document.getElementById('control-mode-select') as HTMLSelectElement | null;
    if (controlModeSelect) {
      controlModeSelect.addEventListener('change', (e) => {
        this.setControlMode((e.target as HTMLSelectElement).value as ControlMode);
      });
    }
    ...
    // Bean model selector
    const beanModelSelect = document.getElementById('bean-model-select') as HTMLSelectElement | null;
    if (beanModelSelect) {
      beanModelSelect.addEventListener('change', async (e) => {
        ...
        try {
          ...
          this.showError('Failed to load bean model: ' + (error as Error).message);
        }
      });
    }
  ...
  async loadModels(): Promise<void> {
    try {
      ...
      // Get base URL from Vite (e.g., '/autoroaster/' in production, '/' in development)
      const baseUrl = import.meta.env.BASE_URL;
      ...
      const phaseEl = document.getElementById('roast-phase');
   ...
   */
  private async loadNeuralController(): Promise<void> {
    try {
      ...
      const metadataUrl = `${baseUrl}onnx_models/controller_metadata.yaml`;
      ...
      const controllerUrl = `${baseUrl}onnx_models/control_policy.onnx`;
      console.log(`Loading ONNX model from: ${controllerUrl}`);
      const controllerSession = await ort.InferenceSession.create(controllerUrl);
      ...
      if (neuralOption) {
        (neuralOption as HTMLOptionElement).disabled = false;
    ...
    for (const line of lines) {
      ...
      if (colonIdx > 0) {
        ...
        if (!value || value === '') {
          ...
          if (!isNaN(Number(value)) && value !== '' && !value.startsWith('0x')) {
  ...
  private initializePreheatState(): Float32Array {
    ...
    // Return state vector: [T_r, T_b, T_air, T_bm, T_atm]
   ...
   */
  private addBackgroundProfileToChart(): void {
   ...
   * Set controller mode
   * 
   * @param mode - 'manual', 'pid', or 'neural'
   */
   ...
   */
  private setupPIDTuningSliders(): void {
    // Heater PID sliders
    const heaterKpSlider = document.getElementById('heater-kp-slider') as HTMLInputElement | null;
    const heaterKiSlider = document.getElementById('heater-ki-slider') as HTMLInputElement | null;
    const heaterKdSlider = document.getElementById('heater-kd-slider') as HTMLInputElement | null;
    ...
    if (heaterKpSlider && heaterKpValue) {
    ...
    if (heaterKiSlider && heaterKiValue) {
    ...
    if (heaterKdSlider && heaterKdValue) {
   ...
   */
  getSetpointFromProfile(currentTime: number): number {
   ...
   * 
   * @param profile - Profile object with times, temps, metadata
   */
  setBackgroundProfile(profile: BackgroundProfile): void {
   ...
   * @param enabled - Whether the mass slider should be enabled
   */
  private setMassSliderEnabled(enabled: boolean): void {
    ...
    try {
      ...
      if (this.controlMode === 'pid' && beansPresent && this.pidController) {
        ...
        const controlActions = await this.neuralController.compute({
          ...
          getSetpoint: (time: number) => this.getSetpointFromProfile(time),
          generateForecast: async (heat: number, fan: number) => {
      ...
      // Cast through 'any' to avoid TypeScript union type issues with ONNX Runtime
  ...
  /**
   * Compute 240-second forecast from current state using current control inputs
   * This is an inner loop that predicts future temperatures over the next 240 seconds
   ...
   */
  async compute60SecondForecast(): Promise<ForecastResult> {
    ...
    const forecastTime: number[] = [];           // Time points in minutes (relative to current simulation time)
    const forecastBeanTemp: number[] = [];       // Predicted bean probe temperatures (T_bm) in °C
    const forecastEnvironmentTemp: number[] = []; // Predicted bean surface temperatures (T_b) in °C
    const forecastRoasterTemp: number[] = [];    // Predicted roaster temperatures (T_r) in °C
    const forecastAirTemp: number[] = [];        // Predicted air temperatures (T_air) in °C
  ...
  private updateStatusDisplay(): void {
    if (this.timeData.length === 0) {
      ...
      const beanTempEl = document.getElementById('bean-temp');
      const envTempEl = document.getElementById('env-temp');
      const roasterTempEl = document.getElementById('roaster-temp');
      const airTempEl = document.getElementById('air-temp');
      const airTempMeasuredEl = document.getElementById('air-temp-measured');
      const roastTimeEl = document.getElementById('roast-time');
      const rateOfRiseEl = document.getElementById('rate-of-rise');
